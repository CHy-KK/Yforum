...
int main()
{
    ...
    while (use) {
        if (curstate == initial) {
            ifv = false;
            cout << "visitir:2;    log in:1;    register:0,please input:";
            int choose;
            cin >> choose;
            cin.clear();
            cin.ignore(10000, '\n');
            ...
            while (true) {
                ...
                if (!ifused) {
                    ...
                    curstate = typechoose;
                    //新用户注册进入数据库
                    Uquery.prepare("INSERT INTO user(ID,PASSWORD,LEVEL,ADMIN,POSTTYPE)"
                                  "VALUES (:ID,:PASSWORD,:LEVEL,:ADMIN,:POSTTYPE)");
                    Uquery.bindValue(":ID", QString::fromStdString(id));
                    Uquery.bindValue(":PASSWORD", QString::fromStdString(password));
                    Uquery.bindValue(":LEVEL", 0);
                    Uquery.bindValue(":ADMIN", 0);
                    Uquery.bindValue(":POSTTYPE", 0);
                    Uquery.exec();
                    break;
                }
            }
            ...
            if (choice == 1) {
                ...
                string com;
                getline(cin, com);
                thispost->addcommet(com, thisuser->getUid(), replyNo, thispost->getPid());
            }
            else if (choice == 5) {
                ...
                cin.clear();
                cin.ignore(10000, '\n');
                int ifdelete = thispost->deletecomment(deleteNo, thisuser->getUid(), thispost->getPid());
                if (ifdelete == 0)
